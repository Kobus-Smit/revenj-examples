...
namespace UseCase1.App.WinForms.Classes
{
    ...
    {
        public static (Submission submission, Customer customer, Form form, FormGroup group, DataTable inputsTable) LoadSubmission(string submissionURI)
        {
            var result = nameof(LoadSubmission).Execute<string, (Submission submission, Customer customer, Form form, FormGroup group, byte[] inputsTableBytes)>(submissionURI);
            return (result.submission, result.customer, result.form, result.group, result.inputsTableBytes.ToDataTable()) ;
        }
